...
int main()
{
	...
	fwork::Vec3 s = desk->getComponent<ecs::SizeComponent>()->getSize();
	...
	for (int i = 0; i < 3; i++)
	{
		...
		entity->getComponent<ecs::TransformComponent>()->scale(1, 1, 1);

		entity->getComponent<ecs::SizeComponent>()->recalculateSize(*barrelVertices, entity->getComponent<ecs::TransformComponent>(), 2);
		fwork::Vec3 thisSize = entity->getComponent<ecs::SizeComponent>()->getSize();
		entity->getComponent<ecs::TransformComponent>()->move(-thisSize.x * (i-1) , s.y/2 + thisSize.y/2 , 0);
